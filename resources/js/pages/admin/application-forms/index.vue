        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="space-y-6">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    />
                    <Button as-child>
                        <Link href="/admin/application_forms/create">Add</Link>
                    </Button>
                </div>
                ...
                <Card>
                    ...
                    <CardContent class="p-0">
                        <div class="divide-y divide-sidebar-border">
                            <div
                                v-for="a in (applicationForms || [])"
                                :key="a.id"
                                class="flex flex-wrap items-center justify-between gap-4 px-6 py-4"
                            >
                                <div class="min-w-0 flex-1">
                                    <p class="truncate font-medium text-foreground">
                                        {{ a.applicant_name }} · {{ a.email }}
                                    </p>
                                    <p class="truncate text-sm text-muted-foreground">
                                        {{ a.course_interest || '—' }} · {{ a.status || '—' }}
                                    </p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <Button variant="outline" size="sm" as-child>
                                        <Link :href="`/admin/application_forms/${a.id}/edit`">
                                            Edit
                                        </Link>
                                    </Button>
                                </div>
