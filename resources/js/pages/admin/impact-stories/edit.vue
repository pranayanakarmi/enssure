...
const props = defineProps({
    impactStory: { type: Object, required: true },
});
...
const currentImageUrl = props.impactStory.image_url ?? null;
...
function submitForm() {
    form.put(`/admin/impact_stories/${props.impactStory.id}`, { forceFormData: true });
}
        ...
        <div class="flex h-full flex-1 flex-col gap-5 overflow-x-auto p-5">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl font-semibold tracking-tight text-foreground">Edit Impact Story</h1>
                        <p class="text-xs text-muted-foreground mt-0.5">{{ impactStory.title }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs text-gray-600">
                    <Type class="h-3.5 w-3.5" />Edit Story
                </div>
            </div>
            ...
            <Card class="border-gray-200 shadow-sm">
                <CardHeader class="border-b border-gray-200 px-5 py-4">
                    ...
                    <CardDescription class="text-xs">Update the impact story information.</CardDescription>
                </CardHeader>
                <CardContent class="px-5 py-5">
                    <form class="flex flex-col gap-5" @submit.prevent="submitForm">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <Input id="title" v-model="form.title" class="h-9 text-sm" placeholder="Story title" />
