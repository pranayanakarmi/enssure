...
function deleteStory() {
    ...
    router.delete(`/admin/impact_stories/${storyToDelete.value.id}`, {
        preserveScroll: true,
        onFinish: () => {
        ...
        <div class="flex h-full flex-1 flex-col gap-5 overflow-x-auto p-5">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl font-semibold tracking-tight text-foreground">Impact Stories</h1>
                        <p class="text-xs text-muted-foreground mt-0.5">Manage individual impact stories.</p>
                    </div>
                </div>
                <Button size="sm" class="h-8 gap-1.5 text-xs" as-child>
                    <Link href="/admin/impact_stories/create">
                        <Plus class="h-3.5 w-3.5" /> Add Story
                    </Link>
                </Button>
            </div>
            ...
            <!-- Stories Table (without Status column) -->
            <Card class="border-gray-200 shadow-sm">
                <CardHeader class="border-b border-gray-200 px-5 py-4">
                    <CardTitle class="text-sm font-semibold">Stories List</CardTitle>
                    <CardDescription class="text-xs">Click Edit to modify a story or Delete to remove.</CardDescription>
                </CardHeader>
                <CardContent class="p-0">
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr v-if="impactStories.length === 0">
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex justify-end gap-2">
                                            <Button
                                                ...
                                                as-child
                                            >
                                                <Link :href="`/admin/impact_stories/${story.id}/edit`">
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="showDeleteModal = false">
            <div class="w-full max-w-md rounded-lg bg-background p-6 shadow-lg">
                ...
                <p class="mt-2 text-sm text-muted-foreground">
                    Are you sure you want to delete “{{ storyToDelete?.title }}”?<br>
                    This action cannot be undone.
                </p>
                <div class="mt-6 flex justify-end gap-2">
                    <Button variant="outline" size="sm" @click="showDeleteModal = false">Cancel</Button>
                    <Button variant="destructive" size="sm" @click="deleteStory" :disabled="isDeleting">
