        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="space-y-6">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    />
                    <Button as-child>
                        <Link href="/admin/training_programs/create">Add</Link>
                    </Button>
                </div>
                ...
                <Card>
                    <CardHeader class="sr-only">
                        <span>Training program list</span>
                    </CardHeader>
                    <CardContent class="p-0">
                        <div class="divide-y divide-sidebar-border">
                            <div
                                v-for="p in (trainingPrograms || [])"
                                :key="p.id"
                            ...
                            >
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="truncate text-sm text-muted-foreground">
                                        {{ p.program_type || '—' }} · {{ p.duration || '—' }} · {{ p.level || '—' }}
                                    </p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <Button variant="outline" size="sm" as-child>
                                        <Link :href="`/admin/training_programs/${p.id}/edit`">
                                            Edit
                                        </Link>
                                    </Button>
                                </div>
