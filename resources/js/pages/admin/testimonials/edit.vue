...
const displayImageUrl = computed(() => imagePreview.value ?? existingImageUrl.value);
        ...
        <div class="flex h-full flex-1 flex-col gap-5 overflow-x-auto p-5">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-xl font-semibold tracking-tight text-foreground">Edit Testimonial</h1>
                        <p class="text-xs text-muted-foreground mt-0.5">{{ testimonial.name }}</p>
                    </div>
                </div>
                <div class="flex items-center gap-1.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs text-gray-600">
                    <ImageIcon class="h-3.5 w-3.5" />Edit Testimonial
                </div>
            </div>
            ...
            <Card class="border-gray-200 shadow-sm">
                <CardHeader class="border-b border-gray-200 px-5 py-4">
                    ...
                    <CardDescription class="text-xs">Update the testimonial information.</CardDescription>
                </CardHeader>
                <CardContent class="px-5 py-5">
                    <form class="flex flex-col gap-5" @submit.prevent="form.post(`/admin/testimonials/${testimonial.id}`, { forceFormData: true })">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <div class="flex flex-wrap gap-3">
                                <div v-if="displayImageUrl" class="relative">
                                    <img :src="displayImageUrl" alt="Preview" class="h-20 w-20 rounded-full border border-gray-200 object-cover" />
