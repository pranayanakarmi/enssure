...
const form = useForm({
    full_name: props.beneficiary.full_name ?? '',
    gender: props.beneficiary.gender ?? '',
    contact_number: props.beneficiary.contact_number ?? '',
    province: props.beneficiary.province ?? '',
    district: props.beneficiary.district ?? '',
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        <Head title="Edit beneficiary" />
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="space-y-6">
                <Heading variant="small" title="Edit beneficiary" :description="beneficiary.full_name" />
                <form class="space-y-6" @submit.prevent="form.put(`/admin/beneficiaries/${beneficiary.id}`)">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <Button variant="outline" as-child>
                            <Link href="/admin/beneficiaries">Back</Link>
                        </Button>
                    </div>
                </form>
            </div>
