...
const hero = computed(() => props.postHero ?? {
    id: null,
    hero_image_url: null,
});
...
<template>
    <AppLayout :breadcrumbs="breadcrumbItems">
        <Head title="Edit Posts Hero Background" />
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="space-y-6">
                <Heading
                    variant="small"
                    title="Posts Hero Background"
                    description="Choose one shared hero background image for all public posts under /posts/*."
                />
                <form
                    class="space-y-6"
                    @submit.prevent="form.put('/admin/posts/hero', { forceFormData: true })"
                >
